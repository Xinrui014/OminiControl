"""
PCB Harmonization v4_subclass — Same as v3.1 (1024) but with ComponentBankV2.1.

Changes from v3.1:
- ComponentBankV2.1: subclass-aware matching, orientation rotation, self-fallback
- v2.1_subclass annotations (adds per-component sub_class field)
- Everything else identical: 1024x1024, same LoRA, same optimizer, same augmentation
"""
import json
import os
import random
...
class PCBHarmonizeDatasetV4(Dataset):
    """
    On-the-fly composite pasting dataset with ComponentBankV2.1.

    Same as V3 (v3.1) except:
    - Uses ComponentBankV2.1 (subclass-aware matching, orientation rotation)
    - v2.1_subclass annotations with per-component sub_class field
    - Self-fallback: if no match, paste original component from same board
    """
    ...
    def __init__(
        ...
        print(f"[DatasetV4] Loading boards from {anno_dir} ({len(anno_files)} files)...")
        ...
        print(f"[DatasetV4] {len(self.boards)} boards, {len(self.samples)} samples "
    ...
    def __getitem__(self, idx):
        ...
        # Decide crop size: zoom (256) or normal (target_w)
        ...
        if crop_size != target_w:
            ...
            # Scale annotations for weight mask
        ...
        for ann in annotations:
            ...
            if result is not None:
                ...
                crop = self.bank.load_crop(
                    entry, rw_int, rh_int,
                    rotation=rotation,
                    resize_jitter=self.resize_jitter,
                )
            else:
                # Self-fallback: use original component from same board
                # original_bbox is in full-board coordinates
                orig_bbox = ann.get("original_bbox")
                if orig_bbox is not None:
                    crop = self.bank.load_self_crop(
                        exclude_board, orig_bbox, rw_int, rh_int,
                        resize_jitter=self.resize_jitter,
                    )
                else:
                    # Crop directly from board_img at crop-relative position
                    abs_x = crop_x + int(rx)
                    abs_y = crop_y + int(ry)
...
# Required import for Path usage in dataset __init__ and test_function
