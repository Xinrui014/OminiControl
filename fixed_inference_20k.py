#!/usr/bin/env python3
"""FIXED inference - properly loads LoRA weights"""
import os
import sys
import json
import numpy as np
...
# FIXED: Load LoRA weights properly using flux_pipe.load_lora_weights()
print(f"Loading LoRA weights from: {checkpoint_path}")
adapter_name = model.adapter_names[2]  # This is "default"
model.flux_pipe.load_lora_weights(
    checkpoint_path,
    weight_name=f"{adapter_name}.safetensors",
    adapter_name=adapter_name
)
print(f"✓ LoRA weights loaded for adapter: {adapter_name}")
